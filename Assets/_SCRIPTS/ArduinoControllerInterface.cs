...
public class ArduinoControllerInterface : MonoBehaviour
{
    ...
    public BikeMovement bikeMovement; //The item we want to affect with our accelerometer
    public AnimationCurve throttleCurve;
    public float input;

    void Start()
    {
        sp = new SerialPort("COM6", 9600, Parity.None, 8, StopBits.One); //Replace "COM4" with whatever port your Arduino is on.
    ...
    void Update()
    {
        ...
        if (cmd.StartsWith("R")) //Got a rotation command
        {
            ...
            input = throttleCurve.Evaluate((accl.y + 360)/720);
